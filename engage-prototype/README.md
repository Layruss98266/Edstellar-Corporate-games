...
|---|---|---|
| **Improv Challenge v3** | `play/improv-challenge.html` | The fullest-featured demo in the prototype. **28 EN prompts + compact 10-prompt packs in Hindi / Arabic / Spanish** (RTL for Arabic). Difficulty + skill tags. Strong/weak example responses. **Configurable total rounds (3-10, default 3)** with a final-winner celebration. **Custom prompt creator** with JSON pack import/export. **Saved session templates** for quick re-runs. **Customisable teams** (inline name/members edit, color swatches, avatar picker). **Live coaching toolbar** with 8 behavior tags per team that auto-nudge criterion scores. **Auto coaching tips** at round end (weakest criterion + linked Edstellar course). **10-badge achievement system** (First Win, Hat-Trick, Comeback Kid, Champion, etc.). **Per-team analytics** with canvas radar + round trend charts + weakest-criterion callout. **EngageCoins economy** + 5 power-ups + 10 plot twists. **Daily streak tracker**. **Share modal** with shareable URL (base64-encoded state in hash), Slack/Teams card text, and embed snippet. **Presenter mode** (`?mode=present`) for big-screen facilitation; **Player mode** (`?mode=play`) for phones. **14-day reinforcement reminder** as a downloadable .ics calendar file. Color-state timer with audio cues. **Keyboard shortcuts** (Space, N, T, F, P/L/R/A/B, Esc). JSON + CSV exports. Print-friendly. Fully client-side · state persists in `localStorage`. |
