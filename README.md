...
| **Improv Challenge v3 (playable)** | `play/improv-challenge.html` | Fully playable in-browser game. 28 EN prompts + 10-prompt packs in Hindi / Arabic / Spanish (RTL for Arabic) × 8 categories with difficulty + skill tags + strong/weak examples. **Configurable total rounds (3-10)** with final-winner celebration. **Custom prompt creator** with JSON pack import/export. **Saved session templates**. **Customisable teams** with inline editor. **Live coaching toolbar** (8 behavior tags that auto-nudge scores). **Auto coaching tips** + linked Edstellar courses at round end. **10-badge achievement system**. **Per-team analytics** with canvas radar + round trend + weakest-criterion callout. EngageCoins economy + 5 power-ups + 10 plot twists. **Daily streak tracker**. **Share modal** (URL + Slack/Teams card + embed snippet). **Presenter mode** (`?mode=present`) and **Player mode** (`?mode=play`). **14-day reinforcement reminder** as downloadable .ics. Color-state timer with audio cues. Keyboard shortcuts. JSON + CSV exports. Print-friendly. localStorage persistence. |
